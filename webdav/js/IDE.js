...
fn: function (html) {
    ...
    self['@classView'] = smalltalk.send(smalltalk.send(html, "_div", []), "_with_", [function () {title = smalltalk.send(smalltalk.PlusMinusIcon || PlusMinusIcon, "_text_", [smalltalk.send(self['@selectedClass'], "_name", [])]);titleBrush = function ($rec) {smalltalk.send($rec, "_with_", [title]);smalltalk.send($rec, "_class_", ["browser-class-name"]);return smalltalk.send($rec, "_css_put_", ["font-size", "1.2em"]);}(smalltalk.send(html, "_span", []));return classMethodButton = function ($rec) {smalltalk.send($rec, "_renderWhenOn_", [function (html) {return smalltalk.send(smalltalk.send(html, "_span", []), "_with_", ["hoge"]);}]);return smalltalk.send($rec, "_renderWhenOff_", [function (html) {return smalltalk.send(smalltalk.send(html, "_span", []), "_with_", ["fuga"]);}]);}(smalltalk.send(smalltalk.ToggleButton || ToggleButton, "_new", []));}]);
    return self;
},
args: ["html"],
source: "renderOn: html\x0a\x09| title titleBrush classMethodButton |\x0a\x09root := html root name:  'class-', selectedClass name.\x0a\x09classView := html div with: [\x0a\x09\x09title := PlusMinusIcon text: selectedClass name.\x0a\x09\x09titleBrush := html span with: title;\x0a\x09\x09\x09\x09class: 'browser-class-name';\x0a\x09\x09\x09\x09css: 'font-size' put: '1.2em'.\x0a\x09\x09classMethodButton := ToggleButton new\x0a\x09\x09\x09renderWhenOn: [ :html | html span with: 'hoge' ];\x0a\x09\x09\x09renderWhenOff: [ :html | html span with: 'fuga' ]\x0a\x09]",
messageSends: ["name:", "root", ",", "name", "with:", "div", "text:", "class:", "css:put:", "span", "renderWhenOn:", "renderWhenOff:", "new"],
referencedClasses: ["PlusMinusIcon", "ToggleButton"]
}),
smalltalk.BrowserForClass);
