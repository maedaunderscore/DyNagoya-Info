...
block         = '[' ws params:blockParamList? ws sequence:sequence? ws ']' {
	          return smalltalk.BlockNode._new()
	          	._parameters_(sequence._placeHolders().concat(params || []))
//	          	._parameters_((params || []))
...
method        = ws 
    "grammar" ws  name:identifier ws ":" ws rule:identifier ws ">" ws selector:identifier ws body:(.+) ws {
    ...
    / "ometa" ws  name:identifier ws body:(.+) ws {
                return smalltalk.OMetaDefineNode._new()
                               ._name_(name)
                               ._body_(["ometa ", name, " ", body.join("")].join(""));
     }
